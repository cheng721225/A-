...
st.markdown(
    "操作流程：**1. 選擇戶別與樓層** ➔ **2. 直接點擊平面圖新增缺失編號** ➔"
...
st.markdown(
    "💡 **提示：直接用滑鼠點擊下方平面圖的任意位置，即可自動新增缺失編號！**"
)
...
if os.path.exists(plan_filename):
  ...
  # 強制將圖片縮小為 0.5 倍（確保 100% 網頁下也能完整瀏覽）
  ...
  # 改為上下獨立區塊：上方放平面圖（100% 寬度不被切斷），下方放控制按鈕
  ...
  st.warning(
      ...
      f" 請將對應的 JPG 檔（例如 `{plan_filename}`）與此程式放在同個資料夾中。"
  )
